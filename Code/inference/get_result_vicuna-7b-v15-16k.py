...
RESULT_PATH = "../result/small_data/vicuna-7b-v15/"
...
specail_types = ["max_flow","graph_coloring","hamiltonian","tsp","eulerian"]
...
def generate_special_prompt(graph_type,Q):
    if graph_type == "shortest_path":
        ...
        prompt = "\nQ: " + Q + "Please provide the answer directly without the reasoning process and output it in JSON format. For example: \{\"length\"\:LENGTH, \"path\": PATH}  \nA: "     
    elif graph_type == "eulerian":
        prompt = "\nQ: " + Q + "Please provide the answer directly without the reasoning process . Yes or No.\nA: "   
    return prompt
def main():
    """
    ...
    sampling_params = SamplingParams(n=2,
                        ...
                        max_tokens = 2048)
    llm = LLM(model="/userhome/nips_2024_graph/benchmark_data/model/vicuna-7b-v1.5-16k",trust_remote_code=True,gpu_memory_utilization = 0.8)        
    ...
    for graph_type, graph_name in graph_types:
        ...
        for dataset_name in file_list:
            ...
            for index,graph in enumerate(json_file):
                ...
                if graph_type in specail_types:
                    dialogs = [
                        [
                    ...
                    dialogs = [
                        ...
                        [
                            {"role": "user", "content": graph_description + "\nQ: " + similarity_q +  "Please output the triplet of this graph in JSON format.For example: \{\"triple\"\:[(node_A,node_B,weight)] \nA: "},
                        ],
                        [
                            {"role": "user", "content": graph_description + generate_special_prompt(graph_type,specail_qa_q)},
                        ],
